...
class ATE_Learner:
    ...
    def estimate_callibration(self,train):
        ...
        Y_a_Pred = Reg_0_X.predict(train[train['Treatment']==1].iloc[:,:self.dim])
        Label_diff = Y_a_Pred -  train[train['Treatment']==1]['Label']
        reg = LinearRegression().fit(train[train['Treatment']==1].iloc[:,:self.dim], Label_diff)
